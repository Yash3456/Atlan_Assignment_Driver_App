...
export default function HomeScreen() {
  ...
  const sendLocationUpdate = async (location: any) => {
    ...
    await axios
      .get(`${process.env.SERVER_URI}/driver/me`, {
        headers: {
  ...
  const getRecentRides = async () => {
    const accessToken = await AsyncStorage.getItem("accessToken");
    const res = await axios.get(`${process.env.SERVER_URI}/driver/get-rides`, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
    });
  ...
  const handleStatusChange = async () => {
    if (!loading) {
      ...
      const changeStatus = await axios.put(
        `${process.env.SERVER_URI}/driver/update-status`,
        {
  ...
  const acceptRideHandler = async () => {
    ...
    await axios
      .post(
        `${process.env.SERVER_URI}/driver/new-ride`,
        {
          userId: userData?.id!,
